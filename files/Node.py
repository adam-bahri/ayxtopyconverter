class NodeElement(object):
    ...
    def __init__(self, node):
        self.tool_id = int(node.attrib['ToolID'])
        ...
        self.engine_settings = node.find("EngineSettings")
        self.macro = self.engine_settings.get('Macro') if self.engine_settings is not None else None

        # Data Cleansing tool
        if self.macro is not None and 'cleanse' in self.macro.lower():
            self.plugin = 'Data Cleansing'
            self.tool = 'Data Cleansing'
